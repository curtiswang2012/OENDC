...
class SoundEngine {
  ...
  playPerfectDodge() {
    ...
    // Time Dilation Whoosh & Harmonic Warp
    const osc = this.ctx.createOscillator();
    const gain = this.ctx.createGain();
    const filter = this.ctx.createBiquadFilter();
  ...
  playParry() {
    ...
    // High Metallic Chime (鏗——！) & Blade Clash Impact
    [1760, 2637, 3520].forEach((freq, idx) => {
      ...
      gain.gain.setValueAtTime(0.5 / (idx + 1), t);
  ...
  playElementalBurst() {
    ...
    // Booming Low-End Sub + High Resonance Shimmer
    const oscSub = this.ctx.createOscillator();
    const gainSub = this.ctx.createGain();
    oscSub.type = 'sawtooth';
    oscSub.frequency.setValueAtTime(90, t);
    oscSub.frequency.exponentialRampToValueAtTime(25, t + 0.5);
    gainSub.gain.setValueAtTime(0.7, t);
    gainSub.gain.exponentialRampToValueAtTime(0.01, t + 0.55);
    ...
    // Starlight Shimmer
    [523.25, 783.99, 1318.5].forEach((freq, idx) => {
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
      osc.type = 'sine';
      osc.frequency.setValueAtTime(freq, t + idx * 0.05);
      osc.frequency.linearRampToValueAtTime(freq * 1.8, t + 0.4);
...
class Enemy {
  constructor(x, y, type = ENEMY_TYPES.CRAWLER) {
    ...
    // Dual-Form Elemental Marks (☀️ Solar Brands / 🌑 Lunar Rot)
    this.solarStacks = 0;
    this.lunarStacks = 0;
    this.elementalTimer = 0;

    // Attack Windup & Parry/Dodge Telegraph Window
    this.isWindup = false;
    this.attackTelegraphTimer = 0;
    this.attackWindupMax = 0.28;
  ...
  takeDamage(amount, isCrit, critMult, particleEngine, form = null) {
    if (this.isDead) return { isDetonation: false, damage: 0 };

    let finalDmg = amount;
    if (this.isStaggered) {
      finalDmg = Math.round(finalDmg * 1.35); // +35% damage during poise break
    }
    if (isCrit) {
    ...
    if (form === 'radiant') {
      if (this.lunarStacks > 0) {
        // Lunar mark struck by Radiant strike -> ECLIPSE NOVA BURST!
        const burstDmg = Math.round(280 + this.lunarStacks * 180 + finalDmg * 0.7);
        finalDmg += burstDmg;
        this.lunarStacks = 0;
        this.solarStacks = 0;
        this.elementalTimer = 0;
        ...
        this.elementalTimer = 5.5;
      }
    } else if (form === 'shadow') {
      if (this.solarStacks > 0) {
        // Solar mark struck by Shadow strike -> ECLIPSE NOVA BURST!
        const burstDmg = Math.round(280 + this.solarStacks * 180 + finalDmg * 0.7);
        finalDmg += burstDmg;
        this.solarStacks = 0;
        this.lunarStacks = 0;
        this.elementalTimer = 0;
  ...
  stagger(duration, particleEngine) {
    this.isStaggered = true;
    this.staggerTimer = duration;
    this.isWindup = false;
    this.attackTelegraphTimer = 0;
    this.attackCooldownTimer = duration + 0.5;
    particleEngine.addFloatingText(this.x, this.y, 'STAGGERED', 'stagger');
    particleEngine.emitParrySparks(this.x, this.y);
  }
  ...
  update(dt, player, lightingEngine, particleEngine, allProjectiles) {
    ...
    // Update Elemental Marks Timer
    if (this.elementalTimer > 0) {
    ...
    if (dist < 580) {
      if (this.type === ENEMY_TYPES.CASTER) {
        ...
        // Windup & Cast Dark Orb
        if (this.attackCooldownTimer <= 0 && dist < 420) {
          if (!this.isWindup) {
            this.isWindup = true;
            this.attackTelegraphTimer = this.attackWindupMax * 1.3;
          } else {
        ...
        // Melee Attack Windup & Strike (Parry / Dodge Telegraph Window)
        if (dist <= this.radius + player.radius + 24) {
          if (this.attackCooldownTimer <= 0 && !this.isWindup) {
            this.isWindup = true;
            this.attackTelegraphTimer = this.attackWindupMax;
          } else if (this.isWindup) {
            this.attackTelegraphTimer -= dt;
            if (this.attackTelegraphTimer <= 0) {
              this.isWindup = false;
              this.attackCooldownTimer = this.type.attackCooldown;
              if (Math.hypot(player.x - this.x, player.y - this.y) <= this.radius + player.radius + 28) {
  ...
  render(ctx, cameraX, cameraY) {
    ...
    // 1. Attack Telegraph Indicator (Parry & Perfect Dodge Flash)
    if (this.isWindup) {
      const pulse = Math.sin(Date.now() * 0.03) * 0.5 + 0.5;
      ctx.beginPath();
      ctx.arc(0, 0, this.radius + 12 + pulse * 6, 0, Math.PI * 2);
      ctx.strokeStyle = '#ef4444';
      ctx.lineWidth = 2.5;
      ctx.stroke();

      // Telegraph Danger Glint
    ...
    ctx.beginPath();
    ctx.arc(0, 0, this.radius, 0, Math.PI * 2);
    ctx.fillStyle = this.isStaggered ? '#475569' : this.type.color;
    ctx.fill();
    ctx.strokeStyle = this.isFrenzied ? '#ef4444' : (this.isStaggered ? '#fde047' : '#334155');
    ctx.lineWidth = 2;
    ctx.stroke();
    ...
    // 6. Dual-Form Elemental Marks Badges
    if (this.solarStacks > 0 || this.lunarStacks > 0) {
      let markText = '';
      if (this.solarStacks > 0) markText = `☀️×${this.solarStacks}`;
      else if (this.lunarStacks > 0) markText = `🌑×${this.lunarStacks}`;

      ctx.fillStyle = this.solarStacks > 0 ? '#ffd700' : '#c084fc';
      ctx.font = 'bold 10px Outfit, sans-serif';
      ctx.fillText(markText, 0, this.radius + 12);
    }

    // 7. Mini Health Bar above monster
    if (this.hp < this.maxHp) {
      ...
      ctx.fillStyle = this.isStaggered ? '#fde047' : '#ef4444';
...
class Player {
  ...
  getCritRate() {
    if (this.hyperchargeTimer > 0) return 1.0; // 100% Guaranteed Crit after Perfect Dodge
  ...
  getCritMultiplier() {
    ...
    if (this.hyperchargeTimer > 0) {
      mult += 0.8; // +80% Crit DMG on Hypercharge
    }
  ...
  toggleForm(particleEngine) {
    ...
    if (this.form === FORMS.RADIANT) {
      particleEngine.emitShockwaveRing(this.x, this.y, 140, '#ffd700', 0.5);
      particleEngine.emitSparks(this.x, this.y, '#ffd700', 16, 200);
    } else {
      particleEngine.emitShockwaveRing(this.x, this.y, 140, '#9400d3', 0.5);
      particleEngine.emitShadowWisps(this.x, this.y, 18);
    }

    // Check Peak Cataclysm Shift (When Energy >= 100)
    const curEnergy = this.form === FORMS.RADIANT ? this.radiantEnergy : this.shadowEnergy;
    if (curEnergy >= 100) {
      if (this.form === FORMS.RADIANT) this.radiantEnergy = 0;
      else this.shadowEnergy = 0;

      // Trigger Cataclysm Shockwave Area
      particleEngine.addShake(15);
      particleEngine.emitShockwaveRing(this.x, this.y, 240, this.form === FORMS.RADIANT ? '#ffd700' : '#8a2be2', 0.6);
      particleEngine.emitSparks(this.x, this.y, '#ffffff', 30, 280);
      audio.playFormShift(true);
      ...
      return {
        ...
        y: this.y,
        radius: 240,
        damage: this.getAttackDamage() * 3.5,
        form: this.form
      };
    } else {
  ...
  // --- Dodge Roll & Perfect Dodge (極限閃避) ---
  dodge(targetAngle, particleEngine, nearbyEnemies = [], incomingProjectiles = []) {
    if (this.dodgeCooldown > 0 || this.isDodging || this.isDead) return null;
    ...
    const dodgeSpeed = 520;
    this.dodgeVx = Math.cos(targetAngle) * dodgeSpeed;
    this.dodgeVy = Math.sin(targetAngle) * dodgeSpeed;

    // Check for Perfect Dodge (極限閃避判定)
    let isPerfect = false;

    // 1. Nearby attacking enemy in windup or close strike
    for (const en of nearbyEnemies) {
      if (en.isDead) continue;
      const d = Math.hypot(en.x - this.x, en.y - this.y);
      if (d < 140 && (en.isWindup || en.attackTelegraphTimer > 0 || d < en.radius + this.radius + 30)) {
        isPerfect = true;
        break;
      }
    }

    // 2. Incoming hostile projectile in trajectory
    if (!isPerfect) {
    ...
    if (isPerfect) {
      this.hyperchargeTimer = 3.5;
      if (this.form === FORMS.RADIANT) {
        this.radiantEnergy = Math.min(this.maxEnergy, this.radiantEnergy + 30);
      } else {
        this.shadowEnergy = Math.min(this.maxEnergy, this.shadowEnergy + 30);
      }

      particleEngine.createHologramDecoy(this.x, this.y, this.form === FORMS.RADIANT ? 'radiant' : 'shadow', this.radius);
      particleEngine.emitShockwaveRing(this.x, this.y, 140, this.form === FORMS.RADIANT ? '#38bdf8' : '#c084fc', 0.45);
      particleEngine.addFloatingText(this.x, this.y - 25, 'PERFECT DODGE', 'perfect_dodge');
  ...
  triggerBasicAttack(targetX, targetY, particleEngine) {
    ...
    if (this.hyperchargeTimer > 0) {
      this.hyperchargeTimer = 0;
      particleEngine.emitSparks(this.x, this.y, '#ffd700', 25, 240);
      particleEngine.emitShockwaveRing(this.x, this.y, 90, '#ffd700', 0.2);
    }
  ...
  update(dt, inputState, lightingEngine, particleEngine) {
    ...
    // 1. Timers
    if (this.hyperchargeTimer > 0) this.hyperchargeTimer -= dt;
    if (this.attackCooldown > 0) this.attackCooldown -= dt;
    ...
    try {
      ...
      const data = {
        ...
        citadel: {
          ...
          greenhouseRations: citadel.greenhouseRations ?? 45,
          lastGreenhouseUpdate: citadel.lastGreenhouseUpdate ?? Date.now(),
          towerMaxFloor: citadel.towerMaxFloor ?? 1,
          siegeMaxWave: citadel.siegeMaxWave ?? 0,
          facilities: citadel.facilities || { smeltery: 1, greenhouse: 1, bastion_wall: 1, workshop: 1, barracks: 1 }
        }
      };
  ...
  static load(player, companion, citadel, arsenal, user = null) {
    try {
      ...
      if (data.player) {
        ...
        const eqId = data.player.weaponId;
        if (eqId && arsenal) {
          const w = arsenal.allWeapons.find(item => item.id === eqId);
          if (w) {
      ...
      if (data.citadel) {
        ...
        citadel.greenhouseRations = data.citadel.greenhouseRations ?? 45;
        citadel.lastGreenhouseUpdate = data.citadel.lastGreenhouseUpdate ?? Date.now();
        citadel.towerMaxFloor = data.citadel.towerMaxFloor ?? 1;
        citadel.siegeMaxWave = data.citadel.siegeMaxWave ?? 0;
        if (data.citadel.facilities) {
          citadel.facilities = { ...citadel.facilities, ...data.citadel.facilities };
        }
        citadel.updateGreenhouse();
      }
...
const AI_MERCENARIES = [
  {
    id: 'ai_roland',
    name: '聖堂守衛·羅蘭',
    title: '【初光聖誓者】',
    role: '光輝近衛 (坦克/護盾)',
    avatar: { icon: '🛡️', color: '#ffd700' },
    form: 'radiant',
    weaponId: 'ssr_dawnbreaker',
    level: 3,
    hp: 1200,
    maxHp: 1200,
    color: '#ffd700'
  },
  {
    ...
    title: '【暗夜獵手】',
    role: '暗影刺殺 (高爆發/瞬移)',
    avatar: { icon: '🌑', color: '#c084fc' },
    form: 'shadow',
    weaponId: 'sr_shadow_daggers',
    level: 3,
    hp: 900,
    maxHp: 900,
    color: '#c084fc'
  },
  {
    ...
    title: '【破曉使徒】',
    role: '神聖貫通 (長槍/範圍控制)',
    avatar: { icon: '🔱', color: '#38bdf8' },
    form: 'radiant',
    weaponId: 'sr_aurora_spear',
    level: 3,
    hp: 1050,
    maxHp: 1050,
    color: '#38bdf8'
  },
  {
    ...
    title: '【深淵征服者】',
    role: '撼地破盾 (重錘/巨額打擊)',
    avatar: { icon: '🔨', color: '#f87171' },
    form: 'shadow',
    weaponId: 'sr_abyss_hammer',
    level: 3,
    hp: 1350,
    maxHp: 1350,
    color: '#f87171'
...
class MultiplayerNetworkEngine {
  ...
  initHttpRelay() {
    ...
    const checkServer = async () => {
      try {
        const resp = await fetch(`${this.serverBaseUrl}/api/mp/rooms`, { method: 'GET', mode: 'cors' });
        if (resp.ok) {
  ...
  startHttpPolling() {
    ...
    const poll = async () => {
      if (this.serverAvailable && this.serverBaseUrl) {
        try {
          const url = `${this.serverBaseUrl}/api/mp/poll?since=${this.lastServerPollTime}&peerId=${this.peerId}`;
          const resp = await fetch(url, { method: 'GET', mode: 'cors' });
          if (resp.ok) {
            const data = await resp.json();
            if (data) {
              if (data.timestamp) this.lastServerPollTime = data.timestamp;
  ...
  sendPacket(type, payload = {}) {
    ...
    // 3. Post to HTTP Relay Server if available
    if (this.serverAvailable && this.serverBaseUrl) {
      fetch(`${this.serverBaseUrl}/api/mp/send`, {
        method: 'POST',
  ...
  addAiMercenary(mercenaryId = null) {
    if (!this.currentRoom || !this.isHost) return false;
    if (this.currentRoom.players.length >= this.currentRoom.maxPlayers) return false;

    let candidate = null;
    if (mercenaryId) {
      candidate = AI_MERCENARIES.find(m => m.id === mercenaryId);
    } else {
      const existingIds = new Set(this.currentRoom.players.filter(p => p.isAi).map(p => p.aiData?.id));
      candidate = AI_MERCENARIES.find(m => !existingIds.has(m.id)) || AI_MERCENARIES[0];
    }

    if (!candidate) return false;

    const peerId = 'ai_' + Math.random().toString(36).substring(2, 8);
    const aiSlot = {
      peerId,
      user: {
    ...
    this.currentRoom.players.push(aiSlot);
    this.broadcastRoomState();
    return true;
  }

  removePlayerSlot(peerId) {
    if (!this.currentRoom || !this.isHost) return false;
    const idx = this.currentRoom.players.findIndex(p => p.peerId === peerId);
    if (idx !== -1 && !this.currentRoom.players[idx].isHost) {
  ...
  fillPartyWithAi() {
    if (!this.currentRoom || !this.isHost) return false;
    let count = 0;
    while (this.currentRoom.players.length < this.currentRoom.maxPlayers) {
      if (this.addAiMercenary()) {
        count++;
      } else {
        break;
      }
  ...
  quickMatch(zoneId = 'barren_wastes') {
    // 1. Check if an active open human room exists in public channel
    const available = Array.from(this.publicRooms.values()).find(r => r.playerCount < r.maxPlayers && r.status === 'lobby');
    if (available) {
      this.joinRoomByCode(available.code);
      return { type: 'join', room: available };
    }

    // 2. Otherwise auto create a room and assemble 2 AI companions
    const room = this.createRoom('破曉討伐先鋒隊', 4, '', zoneId);
    this.addAiMercenary('ai_roland');
    this.addAiMercenary('ai_sylvia');
    return { type: 'create_ai', room };
  }

  createGuildNpcRoom(guildType = 'radiant') {
    if (guildType === 'shadow') {
      const room = this.createRoom('【暗夜深淵暗殺組】', 4, '', 'blood_mire');
      this.addAiMercenary('ai_sylvia');
      this.addAiMercenary('ai_valdor');
      return room;
    } else {
      const room = this.createRoom('【聖堂先鋒遠征隊】', 4, '', 'barren_wastes');
      this.addAiMercenary('ai_roland');
      this.addAiMercenary('ai_elena');
      return room;
    }
  }
...
class MinimapEngine {
  ...
  render(dungeon, player, companion, enemies, boss, remotePlayers = null, pings = null) {
    ...
    // 6.2 Ancient Shrines Markers
    if (dungeon.shrines) {
      for (const sh of dungeon.shrines) {
        const sx = toMapX(sh.x);
        const sy = toMapY(sh.y);
        ctx.save();
        ctx.beginPath();
        ctx.arc(sx, sy, sh.isActivated ? 3 : 4.5, 0, Math.PI * 2);
        ctx.fillStyle = sh.isActivated ? '#64748b' : '#ffd700';
        ctx.fill();
        ctx.strokeStyle = sh.isActivated ? '#475569' : '#b45309';
        ctx.lineWidth = 1.2;
        ctx.stroke();
        ctx.restore();
      }
    ...
    // 6.3 Cursed Chests Markers
    if (dungeon.cursedChests) {
      for (const cc of dungeon.cursedChests) {
        const cx = toMapX(cc.x);
        const cy = toMapY(cc.y);
        ctx.save();
        ctx.beginPath();
        ctx.arc(cx, cy, 4, 0, Math.PI * 2);
        ctx.fillStyle = cc.state === 'cleared' ? '#ffd700' : '#ef4444';
        ctx.fill();
        ctx.strokeStyle = '#fff';
        ctx.lineWidth = 1;
        ctx.stroke();
    ...
    // 6.4 Ascent Portal Markers (天梯登頂傳送陣)
    if (dungeon.ascentPortals) {
      for (const ap of dungeon.ascentPortals) {
        const ax = toMapX(ap.x);
        const ay = toMapY(ap.y);
        ctx.save();
        ctx.beginPath();
    ...
    // 6.45 Citadel Siege Core & Turrets Radar Markers (血月守城要塞標記)
    if (dungeon.isSiegeMode) {
      // Citadel Core
      if (dungeon.citadelCore) {
        const cx = toMapX(dungeon.citadelCore.x);
        const cy = toMapY(dungeon.citadelCore.y);
        ctx.save();
        ctx.beginPath();
        ctx.arc(cx, cy, this.isExpanded ? 8 : 6, 0, Math.PI * 2);
      ...
      // Turrets
      if (dungeon.turrets) {
        for (const t of dungeon.turrets) {
          const tx = toMapX(t.x);
          const ty = toMapY(t.y);
          ctx.save();
          ctx.beginPath();
